...
const PDFUpload: React.FC<{
  ...
  const uploadFile = async (
    ...
    try {
      ...
      const { url } = await uploadToVercelBlob(file, setUrl, urlKey);
      if (url) {
        ...
        console.error("Failed to upload: No URL returned.");
      }
    } catch (error) {
      console.error('Error during the upload process:', error);
      setUrl(null); // Optionally handle the error, e.g., by resetting the URL state to null
    }
  };
  ...
  return (
    <div className="flex flex-col items-center gap-[12px] relative">
      <div className="flex-col items-start flex gap-[4px] relative self-stretch w-full flex-[0_0_auto]  ">
        ...
        <label
          htmlFor="work-deck"
          className="items-center flex gap-[4px] relative self-stretch w-full flex-[0_0_auto] cursor-pointer hover:bg-gray-50 hover:rounded-xl"
        >
          <div className="flex items-center justify-center gap-[6px] px-[40px] py-[16px] relative flex-1 grow rounded-[8px] border border-dashed border-gray-4">
            <FontAwesomeIcon
              icon={faPlus}
      ...
      <div className="flex-col items-start flex gap-[4px] relative self-stretch w-full flex-[0_0_auto]">
        <div className="relative w-fit font-normal text-gray-500 text-xs tracking-[0.24px] leading-[20px] whitespace-nowrap">
          CV
        </div>
        <label
          htmlFor="cv-deck"
          className="items-center flex gap-[4px] relative self-stretch w-full flex-[0_0_auto] cursor-pointer"
        >
          <div className="flex items-center justify-center gap-[6px] px-[40px] py-[16px] relative flex-1 grow rounded-[8px] border border-dashed border-gray-4 hover:bg-gray-50 hover:rounded-xl">
            <FontAwesomeIcon
              icon={faPlus}
