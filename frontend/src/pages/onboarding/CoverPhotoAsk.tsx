...
// working with vercel blob to upload
// import React, { useState } from 'react';
// import { useUser } from '@clerk/clerk-react';
// import { FontAwesomeIcon } from '@fortawesome/react-fontawesome';
// import { faImage } from '@fortawesome/free-solid-svg-icons';
...
//     // Define the maximum file size you want after resizing, e.g., 5MB
//     const maxFileSize = 5 * 1024 * 1024;

//     // First, resize the image if necessary
//     resizeImage(file, maxFileSize)
//       .then((resizedFile) => {
//         // Ensure the resized file is of File type for upload compatibility
//         const fileToUpload = new File([resizedFile], resizedFile.name, {
//           type: resizedFile.type,
//         });

//         // Then, upload the resized image to Vercel Blob
//         uploadToVercelBlob(
//           fileToUpload,
//           (url) => {
//             setCoverPhoto(url); // Update your state with the new image URL
//             localStorage.setItem('coverPhotoUrl', url); // Store the URL in localStorage
//           },
//           'coverPhotoUrl' // The key under which to store the URL in localStorage
//         ).catch((error) => {
...
const CoverPhotoUpload = () => {
  const { user } = useUser();
  const navigate = useNavigate();
  // Initialize coverPhoto state with the value from local storage if it exists
  ...
  const handleCoverPhotoUpload = async (
    ...
    resizeImage(file, maxFileSize)
      .then((resizedFile) => {
        ...
        uploadToVercelBlob(fileToUpload, setCoverPhoto, 'coverPhotoUrl')
          .then(() => {
            console.log(
              'Image uploaded and URL set in state and localStorage.'
            );
          })
          .catch((error) => {
            console.error('Error uploading resized file:', error);
