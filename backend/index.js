...
app.post('/generate-upload-url', async (req, res) => {
  ...
  try {
    const { url, fields } = await createPresignedPost(s3Client, {
      Bucket: process.env.BUCKET_NAME, // Use BUCKET_NAME from your .env
      Key: key,
      Conditions: [
    ...
    // Modify the url to include the bucket name in its path, if necessary
    const modifiedUrl = `https://${process.env.BUCKET_NAME}.s3.${process.env.BUCKET_REGION}.amazonaws.com`;

    // Return the modified URL and fields for the presigned post
    res.json({ url: modifiedUrl, fields });
  } catch (error) {
